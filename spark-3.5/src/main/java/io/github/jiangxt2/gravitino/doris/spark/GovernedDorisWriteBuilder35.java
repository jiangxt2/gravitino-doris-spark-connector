...
final class GovernedDorisWriteBuilder35 implements WriteBuilder, SupportsTruncate {
  ...
  private final DorisWriteSchemaCompatibility.Validator validator;

  GovernedDorisWriteBuilder35(WriteBuilder delegate, DorisWritePolicy writePolicy) {
    this(delegate, writePolicy, DorisWriteSchemaCompatibility.Validator.none());
  }
  ...
  private static final class ValidatingBatchWrite implements BatchWrite {
    ...
    private ValidatingBatchWrite(
        BatchWrite delegate, DorisWriteSchemaCompatibility.Validator validator) {
      this.delegate = Objects.requireNonNull(delegate, "delegate");
  ...
  private static final class ValidatingDataWriterFactory
    ...
    private ValidatingDataWriterFactory(
        DataWriterFactory delegate, DorisWriteSchemaCompatibility.Validator validator) {
      this.delegate = Objects.requireNonNull(delegate, "delegate");
      this.validator = validator;
    }
  ...
  private static final class ValidatingDataWriter implements DataWriter<InternalRow> {
    ...
    private ValidatingDataWriter(
        DataWriter<InternalRow> delegate, DorisWriteSchemaCompatibility.Validator validator) {
      this.delegate = Objects.requireNonNull(delegate, "delegate");
      this.validator = validator;
    }
